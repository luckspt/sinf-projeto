...
sudo iptables -A INPUT -s gcc_ip_address -p icmp --icmp-type echo-request -j ACCEPT
...
sudo iptables -A OUTPUT -d gcc_ip_address -p icmp --icmp-type echo-reply -j ACCEPT

# Allow SSH from and to the "gcc" machine
# -p tcp specifies the protocol
# --dport 22 specifies the destination port
sudo iptables -A INPUT -s gcc_ip_address -p tcp --dport 22 -j ACCEPT

# --sport 22 specifies the source port 
sudo iptables -A OUTPUT -d gcc_ip_address -p tcp --sport 22 -j ACCEPT


# Allow connections from any origin to myCloud server
sudo iptables -A OUTPUT -p tcp --sport myCloud_port -j ACCEPT



# Allow ping to machines in the local subnet with mask 255.255.254.0
sudo iptables -A OUTPUT -p icmp --icmp-type echo-request -d 255.255.254.0/23 -j ACCEPT
